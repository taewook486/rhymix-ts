...
export default async function ModuleDetailPage({
  ...
  // config 는 JSON 컬럼이라 읽는 모양만 좁혀서 쓴다.
  const grantConfig =
    instance.config && typeof instance.config === 'object' && 'grant' in instance.config
      ? ((instance.config as { grant?: { permission?: string; groupIds?: number[] } }).grant ?? null)
      : null;
            ...
            {instance.config && typeof instance.config === 'object' && 'grant' in instance.config ? (
              <dl className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <dd className="text-sm font-medium text-zinc-900">
                    {grantConfig?.permission || '미설정'}
                  </dd>
                </div>
                <div>
                  <dt className="text-sm text-zinc-500">접근 그룹</dt>
                  <dd className="text-sm font-medium text-zinc-900">
                    {grantConfig?.groupIds?.length
                      ? grantConfig.groupIds.join(', ')
                      : '전체'}
                  </dd>
                </div>
              </dl>
