...
- 2026-06-21 (run, e2e 검증 + 사전 존재 결함 6건 발견·수정): 이전 세션에서 작성만 되고 미실행 상태였던 `apps/web/e2e/notification.spec.ts`를 Postgres 가용 환경에서 실제 실행. 최초 실행은 즉시 실패했고, 디버깅 과정에서 SPEC-NOTIFICATION-001 범위를 넘어서는 6건의 사전 존재 프로덕션 결함을 순차적으로 발견·수정함(모두 mock 기반 단위 테스트로는 발견 불가능했던 항목 — 이번이 알림 기능에 대한 최초의 실제 풀스택 검증):
  1. `packages/board/src/feed/*.ts`(index.ts/build-feed.ts/autodiscovery.ts/resolve-feed.ts)의 relative import가 `.js` 확장자를 사용해 Turbopack이 ESM 모듈로 resolve하지 못하고 dev 서버 부팅 자체가 실패(SPEC-FEED-001 결함, `feed.spec.ts`가 `CI_E2E` 가드로 항상 skip되어 미발견). 확장자 제거로 수정.
  2. `packages/document/src/document.ts`·`packages/comment/src/service.ts`의 `sanitizeHtml`이 `require('isomorphic-dompurify')`를 사용해 동일하게 Turbopack ESM 런타임에서 `ReferenceError: require is not defined` 발생 → 문서/댓글 생성이 실서버에서 전부 실패. `await import()` 기반 비동기 함수로 전환(packages/page/src/service.ts의 기존 올바른 패턴과 동일).
  3. (2)의 수정 후에도 `isomorphic-dompurify`(jsdom)가 자체 `default-stylesheet.css`를 `__dirname` 기준 `fs.readFileSync`로 로드하는데, Turbopack이 번들링 시 `__dirname`을 가상 경로(`/ROOT`)로 치환해 ENOENT 발생. `apps/web/next.config.ts`의 `serverExternalPackages`에 `isomorphic-dompurify`/`jsdom` 추가로 번들링 자체를 우회.
