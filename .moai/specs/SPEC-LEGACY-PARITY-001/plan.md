...
**depends_on 이력 — 부모 엣지 제거 (v0.4.0; 감사 D8 서술의 정정).** v0.3.0까지 이 SPEC은
frontmatter에 `depends_on: [SPEC-LEGACY-PARITY-000]`을 선언했고, 감사 D8(v0.3.0 서술)은 부모의
`draft` status를 "run 킥오프 시 wait/override/abort로 풀 조건"으로 기록했다. **그 서술은
틀렸다 — 그 조건은 스케줄링 불편이 아니라 교착(deadlock)이다.** depends_on 사전 점검은
`completed`만 충족으로 본다. 그런데 부모를 이 시리즈가 끝나기 전에 completed로 표시하는 것은
불가능하다: REQ-LGP-001은 시리즈가 정확히 6개 영역 SPEC(001~006)으로 구성될 것을 요구하는데
디스크에는 현재 000·001만 존재하고, 부모 자신의 §4도 "실제 구현은 전부 영역 SPEC 001~006에서
이루어진다"고 못 박는다. REQ-LGP-003(전건 판정 미달인 영역 SPEC은 완료 불가)과 REQ-LGP-006
(영역 SPEC completed 도달 시 승계 전환 발화)도 영역 SPEC의 완료를 전제로 작동한다. 즉 001은
000이 completed여야만 시작할 수 있고 000은 001~006이 completed여야만 completed가 될 수 있다 —
시작 불가능한 대기 사이클이며, 형제 002~006이 작성되면 전부 같은 벽에 부딪힌다.

**모델링 수정.** 이 스키마에서 `depends_on`은 "이 SPEC이 소비하는 산물로, 먼저 완료돼야 하는
것"을 뜻한다. 부모는 그 형태가 아니다 — 시리즈 전체에 걸쳐 시효 없이 시행되는 **통치 문서**다.
이 SPEC은 부모의 규약(REQ-LGP-001~008)을 준수하며, 소비하는 산물인 크롤 기준선
(`.moai/reports/legacy-admin-map/`)은 부모의 frontmatter status와 무관하게 이미 디스크에 존재하는
파일 산출물이다. v0.4.0에서 엣지를 제거했다(spec.md §7 — 관계는 규약 준수로 본문에 보존).
선언이 없으므로 depends_on 사전 점검은 자명하게 통과한다. **002~006 작성자는 같은 엣지를
재도입하지 말 것** — 통치 문서는 완료 게이트 의존성의 형태가 아니다. 1~2차 감사가 이 결함을
놓친 것도 모델링 오류가 "런타임 스케줄링 조건"으로 위장했기 때문이다.
