...
- 2026-08-07: 카테고리 4(기타 고립 실패) 재검증. §4.4에 파일명이 특정된 10개 파일(install-validate/db-validator/install-actions/auth-actions/admin-helpers/diagnostics/widgets-render/feed-page/widgets-content/widgets-login-info) 전수 재실행 — 115/117 테스트 통과(2 skip), 실패 0건. 파일 미특정이던 "이미지 포맷 에러 1건"·"업로드 토큰 서명 불일치 1건"도 에러 시그니처로 소스를 역추적해 재실행 — `image-pipeline.test.ts`(이미지 포맷 에러는 graceful-handling을 검증하는 의도된 테스트로 확인, 실패 아님), `upload-token`/`attachment` 관련 5개 파일(48/48 통과) 모두 정상. 즉 카테고리 4 원본 목록은 사실상 전건 해소됨(그동안의 다른 SPEC 작업으로 부수적으로 해소된 것으로 추정, REQ-TDEBT-013 준용 — 원인 추정 없이 재실행 결과만 기록). "테스트 타임아웃 1건"은 원본부터 파일 미특정이라 이번에도 특정 못함. 별도로, `pnpm exec vitest run --reporter=verbose`(275파일 전체) 재확인에서 카테고리 4 목록에 없던 신규 실패 4파일/5테스트를 발견 — 개별 격리 재실행으로 판별한 결과 4건은 WSL2 병렬 자원 경합에 의한 가짜 실패(`packages/board/src/index.test.ts` A-1, `denied-list/page.test.tsx` 2건, `groups/page.test.tsx` AC-C1 — 격리 시 전부 통과), 1건(`default-tab.test.tsx` AC-D1)은 격리 재실행에서도 재현되는 진짜 타임아웃 결함으로 확정 — `user.test.ts`(E-5-1)와 동일한 WSL2 콜드 트랜스파일 비용 근접-초과 패턴(16.3초, 기본 15000ms 초과)이라 동일 처방(해당 테스트 1건에 한해 `testTimeout` 30000ms로 상향)을 적용, 격리 재실행 통과 확인(26.6초). 본 SPEC은 여전히 triage 문서이나, 이번 항목은 `user.test.ts`/`TokenEditor.test.tsx` 선례와 동일하게 별도 커밋으로 직접 수정함(REQ-TDEBT-020 — 카테고리 확정 재유도 금지 원칙에 따라 근본 원인을 재분석하지 않고 기존 처방을 그대로 적용).
