...
# MoAI Security Guardian - Layer 1 (PostToolUse pattern warnings) wrapper.
# Forwards stdin JSON to `moai hook security-scan`, which runs an in-process
# regex scan over the written buffer and emits advisory findings. Regex-only,
# never blocks the edit. Thin forwarder; all logic lives in the moai binary.
...
if command -v moai &> /dev/null; then
    exec moai hook security-scan "$@" 2>>"$MOAI_HOOK_STDERR_LOG"
fi

# Try default ~/go/bin/moai
if [ -f "$HOME/go/bin/moai" ]; then
    exec "$HOME/go/bin/moai" hook security-scan "$@" 2>>"$MOAI_HOOK_STDERR_LOG"
fi

# Try ~/.local/bin/moai (Linux install location)
if [ -f "$HOME/.local/bin/moai" ]; then
    exec "$HOME/.local/bin/moai" hook security-scan "$@" 2>>"$MOAI_HOOK_STDERR_LOG"
fi
